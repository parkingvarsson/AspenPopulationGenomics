...
#SBATCH -A b2012141
#SBATCH -p core
#SBATCH -o gene_diversity_divergence.tremuloides.out
#SBATCH -e gene_diversity_divergence.tremuloides.err
#SBATCH -J gene_diversity_divergence.tremuloides.job
#SBATCH -t 1-00:00:00
...
OutDir="/proj/b2010014/GenomePaper/population_genetics/pan_genome/gene_alpha/tremuloides"
...
echo -e "Name\tDn_c\tLn(D)\tPn\tLn(P)\tDs_c\tLs(D)\tPs\tLs(P)" > $OutDir/tremuloides.gene_alpha.$1.txt
echo -e "Name\tDn_c\tLn(D)\tPn\tLn(P)\tDs_c\tLs(D)\tPs\tLs(P)" > $OutDir/tremuloides.gene_alpha.$1.maf_15.txt
...
do
	...
	Ps=$(awk '$5>0' $OutDir/$name.four_fold.vcf |awk '$5<1'|wc -l)  ###$4 represents the polymorphic sites in tremuloides
	Ps_maf15=$(awk '$5>0.15' $OutDir/$name.four_fold.vcf |awk '$5<0.85'|wc -l)  ###$4 represents the polymorphic sites in tremuloides
	Pn=$(awk '$5>0' $OutDir/$name.zero_fold.vcf |awk '$5<1'|wc -l)
	Pn_maf15=$(awk '$5>0.15' $OutDir/$name.zero_fold.vcf |awk '$5<0.85'|wc -l)
	Ln=$(cat $OutDir/$name.zero_fold.bed |wc -l)
	Ls=$(cat $OutDir/$name.four_fold.bed |wc -l)
	Dn=$(awk '$8=="fixed"' $OutDir/$name.zero_fold.vcf |wc -l)
	Ds=$(awk '$8=="fixed"' $OutDir/$name.four_fold.vcf |wc -l)
	...
	###calculate the Jukes-Cantor corrected divergence
        ...
        echo -e "$name\t$Dn_c\t$Ln\t$Pn\t$Ln\t$Ds_c\t$Ls\t$Ps\t$Ls" >> $OutDir/tremuloides.gene_alpha.$1.txt
        echo -e "$name\t$Dn_c\t$Ln\t$Pn_maf15\t$Ln\t$Ds_c\t$Ls\t$Ps_maf15\t$Ls" >> $OutDir/tremuloides.gene_alpha.$1.maf_15.txt
