...
OutDir="/proj/b2010014/GenomePaper/population_genetics/pan_genome/gene_diversity_divergence/tremuloides"

four_fold="/proj/b2010014/GenomePaper/population_genetics/pan_genome/ANGSD/tremuloides/tremuloides_all/thetas/anno_type/four_fold.filter.vcf"
zero_fold="/proj/b2010014/GenomePaper/population_genetics/pan_genome/fst_ngsStat/tremuloides_trichocarpa/all/anno_type/zero_fold.filter.vcf"

echo -e "Name\t4_fold_pi\t0_fold_df" > $OutDir/tremuloides.gene.4pi_0df.$1.txt
...
######df,dxy,fst
	...
	gene_zero_fold_df=$($mean col=8 $OutDir/$name.zero_fold.vcf |cut -f 11)  ###the proportion of fixed differences between P.tremuloides and P.trichocarpa

	echo -e "$name\t$gene_four_fold_pi\t$gene_zero_fold_df" >> $OutDir/tremuloides.gene.4pi_0df.$1.txt
