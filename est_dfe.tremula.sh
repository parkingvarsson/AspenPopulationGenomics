...
input_rscript="/proj/b2011141/pipeline/R/genomic_adaptation_paper/angsd/tremula.real.DFE_input.R"

for file in {zero_fold,intron,intergenic,utr3,utr5}
do
OutDir="/proj/b2010014/nobackup/population_genetics/DFE/tremula/$file/real"
echo $OutDir
if [ ! -d "$OutDir" ]; then
mkdir -p $OutDir
fi
...
do
OutDir="/proj/b2010014/nobackup/population_genetics/DFE/tremula/$anno/real"
...
echo "sfs_input_file  $OutDir/tremula.$anno.real.txt" >> $OutDir/est_dfe.${anno}.site_class-0.txt
...
echo "sfs_input_file  $OutDir/tremula.$anno.real.txt" >> $OutDir/est_dfe.${anno}.site_class-1.txt
...
echo "divergence_file  $OutDir/tremula.$anno.omega.txt" >> $OutDir/est_dfe.${anno}.alpha_omega.txt
