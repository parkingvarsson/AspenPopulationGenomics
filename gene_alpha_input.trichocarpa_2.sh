...
#SBATCH -A b2012141
#SBATCH -p core
#SBATCH -o gene_alpha.trichocarpa.out
#SBATCH -e gene_alpha.trichocarpa.err
#SBATCH -J gene_alpha.trichocarpa.job
#SBATCH -t 1-00:00:00
...
OutDir="/proj/b2010014/GenomePaper/population_genetics/pan_genome/gene_alpha/trichocarpa_tremuloides"
...
echo -e "Name\tDn_c\tLn(D)\tPn\tLn(P)\tDs_c\tLs(D)\tPs\tLs(P)" > $OutDir/trichocarpa.gene_alpha.$1.txt
echo -e "Name\tDn_c\tLn(D)\tPn\tLn(P)\tDs_c\tLs(D)\tPs\tLs(P)" > $OutDir/trichocarpa.gene_alpha.$1.maf15.txt
...
do
	...
	Ps=$(awk '$6>0' $OutDir/$name.four_fold.vcf |awk '$6<1'|wc -l)  ###$6 represents the polymorphic sites in trichocarpa
	Ps_maf15=$(awk '$6>0.15' $OutDir/$name.four_fold.vcf |awk '$6<0.85'|wc -l)  ###$6 represents the polymorphic sites in trichocarpa
	Pn=$(awk '$6>0' $OutDir/$name.zero_fold.vcf |awk '$6<1'|wc -l)
	Pn_maf15=$(awk '$6>0.15' $OutDir/$name.zero_fold.vcf |awk '$6<0.85'|wc -l)
	...
	###calculate the Jukes-Cantor corrected divergence
        ...
        echo -e "$name\t$Dn_c\t$Ln\t$Pn\t$Ln\t$Ds_c\t$Ls\t$Ps\t$Ls" >> $OutDir/trichocarpa.gene_alpha.$1.txt
        echo -e "$name\t$Dn_c\t$Ln\t$Pn_maf15\t$Ln\t$Ds_c\t$Ls\t$Ps_maf15\t$Ls" >> $OutDir/trichocarpa.gene_alpha.$1.maf15.txt
